...
CREATE SCHEMA IF NOT EXISTS FMG_DATA.GOVERNANCE;

-- Create a sensitivity tag
CREATE OR REPLACE TAG FMG_DATA.GOVERNANCE.SENSITIVITY
    ALLOWED_VALUES = 'PUBLIC', 'INTERNAL', 'CONFIDENTIAL', 'PII'
    COMMENT = 'Data sensitivity classification';

-- Apply tags to columns (instant!)
ALTER TABLE FMG_DATA.PRODUCTION.USERS MODIFY COLUMN email 
    SET TAG FMG_DATA.GOVERNANCE.SENSITIVITY = 'PII';
ALTER TABLE FMG_DATA.PRODUCTION.USERS MODIFY COLUMN phone 
    SET TAG FMG_DATA.GOVERNANCE.SENSITIVITY = 'PII';

-- Verify tags
SELECT * FROM TABLE(FMG_DATA.INFORMATION_SCHEMA.TAG_REFERENCES(
    'FMG_DATA.PRODUCTION.USERS', 'TABLE'));
...
CREATE OR REPLACE MASKING POLICY FMG_DATA.GOVERNANCE.EMAIL_MASK AS (val STRING)
RETURNS STRING ->
    CASE 
...
CREATE OR REPLACE MASKING POLICY FMG_DATA.GOVERNANCE.PHONE_MASK AS (val STRING)
RETURNS STRING ->
    CASE 
        WHEN CURRENT_ROLE() IN ('ACCOUNTADMIN', 'FMG_ADMIN') THEN val
...
ALTER TABLE FMG_DATA.PRODUCTION.USERS MODIFY COLUMN email 
    SET MASKING POLICY FMG_DATA.GOVERNANCE.EMAIL_MASK;
ALTER TABLE FMG_DATA.PRODUCTION.USERS MODIFY COLUMN phone 
    SET MASKING POLICY FMG_DATA.GOVERNANCE.PHONE_MASK;
...
SELECT user_id, full_name, email, phone FROM FMG_DATA.PRODUCTION.USERS;
